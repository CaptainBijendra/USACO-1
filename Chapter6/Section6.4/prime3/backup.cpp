...
void dfs(int x,int y)
{
	...
	int hash=full,hash_last_row=full,hash_last_col=full,tmp;
	...
	{
		/* y==0 . determine (x-1,4) */
		tmp=change[next_digit[row[x-1]]];
		col[4]=10*col[4]+tmp;
	}

	if(x==3 && y>0)
	{
		tmp=change[next_digit[col[y-1]]];
		row[4]=10*row[4]+tmp;

		hash_last_row &= next_digit[row[4]];
	}
	...
	if(x>0 && y==3)
		hash_last_col &= next_digit[col[4]];
	...
	while(hash>0)
	{
		...
		int cur = change[lowbit];

		if( (hash_last_row & next_digit[col[y]+cur-be]) && (hash_last_col & next_digit[row[x]+cur-be]) )
		{
			sq.d[x][y]=cur;
